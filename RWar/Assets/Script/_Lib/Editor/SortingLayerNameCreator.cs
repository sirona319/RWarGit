...
/// <summary>
/// SortingLayerを定数で管理するクラスを作成するスクリプト
/// </summary>
public static class SortingLayerNameCreator
{
    // 無効な文字を管理する配列
    ...
    /// <summary>
    /// SortingLayerNameを定数で管理するクラスを作成します
    /// </summary>
    [MenuItem(ITEM_NAME)]
    ...
    public static void CreateScript()
    {
        ...
        builder.AppendLine("/// SortingLayerName定数で管理するクラス");
        builder.AppendLine("/// </summary>");
        builder.AppendFormat("public static class {0}", FILENAME_WITHOUT_EXTENSION).AppendLine();
        builder.AppendLine("{");

        foreach (var n in GetSortingLayerNames())
        {
            builder.Append("\t").AppendFormat(@"public const string {0} = ""{1}"";", n, n).AppendLine();
        }
